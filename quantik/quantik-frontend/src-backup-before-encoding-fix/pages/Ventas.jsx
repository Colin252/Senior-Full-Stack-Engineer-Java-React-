...
import "../styles/Ventas.css";   // ðŸŽ¨ estilos especÃ­ficos de ventas
...
function Ventas() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error(err);
            setMensaje("Error al guardar venta âŒ");
        }
    };
    ...
    const handleEliminar = async (id) => {
        if (window.confirm("Â¿Seguro que deseas eliminar esta venta?")) {
            try {
                ...
                setMensaje("Venta eliminada ðŸ—‘ï¸");
                fetchVentas();
            } catch (err) {
                console.error(err);
                setMensaje("Error al eliminar venta âŒ");
            }
        }
    };
    ...
    return (
        <div className="ventas-page">
            {/* ðŸŽ¥ Video de fondo solo en ventas */}
                    ...
                    {ventas.length > 0 ? (
                        ventas.map((v) => (
                            <tr key={v.id}>
                                ...
                                <td>{v.total}</td>
                                <td>
                                    <button className="editar" onClick={() => handleEditar(v)}>âœï¸</button>
                                    <button className="eliminar" onClick={() => handleEliminar(v.id)}>ðŸ—‘ï¸</button>
                                </td>
                            </tr>
                        ))
                    ) : (
