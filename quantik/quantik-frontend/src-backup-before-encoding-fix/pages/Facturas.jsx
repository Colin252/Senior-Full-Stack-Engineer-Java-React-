...
function Facturas() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setMensaje("Factura registrada âœ…");
            ...
            setMensaje("Error al guardar factura âŒ");
        }
    };

    // âœ… imprimir con estilo pro
    const handleImprimir = async (facturaId) => {
        try {
            ...
            doc.text(`DirecciÃ³n: ${factura.direccion || "-"}`, 14, 72);
            doc.text(`MÃ©todo de Pago: ${factura.metodoPago || "-"}`, 14, 80);
            doc.text(`Notas: ${factura.notas || "-"}`, 14, 88);

            doc.text(`NÃºmero: ${factura.numero}`, 150, 40);
            ...
            if (factura.productos && factura.productos.length > 0) {
                const tabla = factura.productos.map((p) => [
                    p.nombre,
                    p.cantidad || 1,
                    `â‚¡${p.precio.toLocaleString("es-CR")}`,
                    `â‚¡${((p.cantidad || 1) * p.precio).toLocaleString("es-CR")}`
                ]);
            ...
            doc.text(`TOTAL: â‚¡${factura.montoTotal.toLocaleString("es-CR")}`, 14, finalY);

            // ðŸ”¹ Guardar
            doc.save(`factura_${factura.id}.pdf`);
        } catch (err) {
            console.error("Error al imprimir factura:", err);
            alert("Error al generar la factura en PDF âŒ");
        }
    };
    ...
    const handleEliminarFactura = async (facturaId) => {
        ...
        try {
            ...
            setMensaje("Factura eliminada âŒ");
        } catch (err) {
            console.error("Error al eliminar factura:", err);
            setMensaje("Error al eliminar factura âŒ");
        }
    };
...
<div className="facturas-container">
                ...
                <form onSubmit={handleSubmit} className="form-factura">
                    <input type="text" placeholder="NÃºmero de factura" value={numero} onChange={(e) => setNumero(e.target.value)} />
                    ...
                    <input type="text" placeholder="MÃ©todo de pago" value={metodoPago} onChange={(e) => setMetodoPago(e.target.value)} />
                    ...
                    <select onChange={(e) => handleAgregarProducto(e.target.value)}>
                        ...
                        {productos.map((p) => (
                            <option key={p.id} value={p.id}>{p.nombre} (â‚¡{p.precio})</option>
                        ))}
                    </select>
                    ...
                    <table className="facturas-tabla">
                        ...
                        {productosSeleccionados.map((p) => (
                            <tr key={p.id}>
                                ...
                                <td>â‚¡{p.precio * p.cantidad}</td>
                                <td><button onClick={() => handleEliminarProducto(p.id)}>âŒ</button></td>
                            </tr>
                        ))}
                        </tbody>
                    </table>
                ...
                {ultimaFactura && (
                    <div className="ultima-factura">
                        <h3>Ãšltima Factura Registrada</h3>
                        <p><strong>NÃºmero:</strong> {ultimaFactura.numero}</p>
                        ...
                        <p><strong>DirecciÃ³n:</strong> {ultimaFactura.direccion}</p>
                        <p><strong>MÃ©todo de Pago:</strong> {ultimaFactura.metodoPago}</p>
                        <p><strong>Notas:</strong> {ultimaFactura.notas}</p>
                        <p><strong>Total:</strong> â‚¡{ultimaFactura.montoTotal}</p>
                        <button className="imprimir" onClick={() => handleImprimir(ultimaFactura.id)}>ðŸ–¨ï¸ Imprimir</button>
                    </div>
                )}
                        ...
                        {facturas.length > 0 ? (
                            facturas.map((f) => (
                                <tr key={f.id}>
                                    <td>{f.id}</td>
                                    <td>{f.fecha}</td>
                                    <td>â‚¡{f.montoTotal}</td>
                                    <td>
                                        <button className="imprimir" onClick={() => handleImprimir(f.id)}>ðŸ–¨ï¸</button>
                                        <button className="eliminar" onClick={() => handleEliminarFactura(f.id)}>âŒ</button>
                                    </td>
                                </tr>
                            ))
                        ) : (
