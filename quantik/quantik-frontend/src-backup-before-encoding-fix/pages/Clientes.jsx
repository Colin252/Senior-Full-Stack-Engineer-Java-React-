...
import "../styles/Clientes.css";   // ðŸŽ¨ estilos
...
function Clientes() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error(err);
            setMensaje("Error al guardar cliente âŒ");
        }
    };
    ...
    const handleEliminar = async (id) => {
        if (window.confirm("Â¿Seguro que deseas eliminar este cliente?")) {
            try {
                ...
                setMensaje("Cliente eliminado ðŸ—‘ï¸");
                fetchClientes();
            } catch (err) {
                console.error(err);
                setMensaje("Error al eliminar cliente âŒ");
            }
        }
    };
                    ...
                    {clientes.length > 0 ? (
                        clientes.map((c) => (
                            <tr key={c.id}>
                                ...
                                <td>{c.telefono}</td>
                                <td>
                                    <button className="editar" onClick={() => handleEditar(c)}>âœï¸</button>
                                    <button className="eliminar" onClick={() => handleEliminar(c.id)}>ðŸ—‘ï¸</button>
                                </td>
                            </tr>
                        ))
                    ) : (
